...
@CrossOrigin
public class VoteItemController  extends Controller {
	
	public static final String DB_TABLE = "vote_item";
	...
	@CrossOrigin
	public void getAll(){
		List<VoteItemModel> models = VoteItemModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			VoteItemModel model = getModel(VoteItemModel.class, "", true);
	...
	public void search(){
		...
		List<VoteItemModel> models = VoteItemModel.dao.find("select * from "+DB_TABLE+" where "+map+" like '%"+key+"%' and del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null && models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
