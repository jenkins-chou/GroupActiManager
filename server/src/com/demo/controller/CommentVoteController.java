...
@CrossOrigin
public class CommentVoteController  extends Controller {
	
	public static final String DB_TABLE = "comment_vote";
	...
	@CrossOrigin
	public void getAllByVote(){
		String vote_id = getPara("vote_id");
		List<CommentActivityModel> models = CommentActivityModel.dao.find("select a.*,b.nickName,b.avatarUrl from "+DB_TABLE+" a,base_user b where a.vote_id = "+vote_id+" and a.user_id = b.id and a.del != 'delete' and b.del != 'delete'");
	...
	@CrossOrigin
	public void getAll(){
		List<CommentVoteModel> models = CommentVoteModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
	...
	public void get(){
		...
		List<CommentVoteModel> models = CommentVoteModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			CommentVoteModel model = getModel(CommentVoteModel.class, "", true);
	...
	public void search(){
		...
		List<CommentVoteModel> models = CommentVoteModel.dao.find("select * from "+DB_TABLE+" where "+map+" like '%"+key+"%' and del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null && models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
