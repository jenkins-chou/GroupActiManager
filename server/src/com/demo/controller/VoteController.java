...
public class VoteController  extends Controller {
	...
	public static final String DB_TABLE = "vote";
	...
	public void getAllByCreator(){
		...
		List<VoteModel> models = VoteModel.dao.find("select * from "+DB_TABLE+" where creator = "+creator+" and del != 'delete'");
	...
	@CrossOrigin
	public void getAll(){
		List<VoteModel> models = VoteModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
	...
	public void get(){
		...
		List<VoteModel> models = VoteModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void search(){
		...
		List<VoteModel> models = VoteModel.dao.find("select * from "+DB_TABLE+" where "+map+" like '%"+key+"%' and del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null && models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
